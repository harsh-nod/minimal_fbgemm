...
constexpr uint32_t hip_calc_xblock_count_base(
    Integer1 num_items,
  ...
  // https://docs.nvidia.com/hip/hip-c-programming-guide/index.html#features-and-technical-specifications
...
// See: hip_calc_xblock_count_base
template <
    typename Integer1,
    typename Integer2,
    std::enable_if_t<
    ...
    std::enable_if_t<
        ...
        bool> = true>
constexpr uint32_t hip_calc_xblock_count(
    Integer1 num_items,
    Integer2 threads_per_block) {
  TORCH_CHECK(
      num_items >= 0,
      "When calculating block counts, the number of items must be positive!");
  return hip_calc_xblock_count_base(num_items, threads_per_block);
}
...
///
/// See hip_calc_xblock_count_base() for details.
template <
    typename Integer1,
    typename Integer2,
    std::enable_if_t<std::is_integral<Integer1>::value, bool> = true,
    std::enable_if_t<std::is_integral<Integer2>::value, bool> = true>
constexpr uint32_t hip_calc_block_count(
    Integer1 num_items,
    Integer2 threads_per_block) {
  // The CUDA specification at
  // https://docs.nvidia.com/hip/hip-c-programming-guide/index.html#features-and-technical-specifications
  ...
  return std::min(
      hip_calc_xblock_count(num_items, threads_per_block), max_blocks);
}
